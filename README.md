...
> A Docker-based multi-stack development seed — PHP/Laravel · Python/Django+FastAPI · Node/NestJS · PostgreSQL · nginx · Mailpit
...
| Node application | Node 22 · NestJS · TypeORM — served at `/nest/` |
...
| Node auth | Session auth via Passport.js + DB-stored bearer tokens (API) |
...
## Working on a single stack

All three application stacks start together with `just up`. When you only want to work on one, **stop the others** — don't comment them out in `docker-compose.yml`.

Commenting out a service breaks Docker Compose because nginx's `depends_on` references all three; it will refuse to start with an undefined service error.

Stopping is safe: nginx keeps running and simply returns `502` for the routes of any stopped stack — the other stacks are unaffected.

```bash
# Stop stacks you're not using
docker compose stop nest python   # work on PHP only
docker compose stop php nest      # work on Python only
docker compose stop php python    # work on NestJS only
...
Postgres, Mailpit, and Adminer always stay up — they're shared by all stacks.
...
### NestJS / TypeORM — at `http://localhost/nest/`

| Feature | How to try it |
|---|---|
| **Landing page** | `http://localhost/nest/` |
| **Session login** | `http://localhost/nest/login` → `admin@seed.local` / `password` |
| **Dashboard** | `http://localhost/nest/dashboard` — protected, DB stats + token list |
| **Contact form** | `http://localhost/nest/contact` — protected, sends email via Mailpit |
| **Public API** | `curl http://localhost/nest/api/v1/status` |
| **Bearer token** | `curl -X POST http://localhost/nest/api/v1/auth/token -H 'Content-Type: application/json' -d '{"email":"admin@seed.local","password":"password"}'` |
| **Private API** | `curl -H 'Authorization: Bearer <token>' http://localhost/nest/api/v1/me` |
| **API Docs** | `http://localhost/nest/api/v1/docs` — Swagger UI (auto-generated by @nestjs/swagger) |
...
| Route | Notes |
|---|---|
| `http://localhost/nest/` | Landing page |
| `http://localhost/nest/login` | `admin@seed.local` / `password` |
| `http://localhost/nest/dashboard` | Protected — DB stats + token list |
| `http://localhost/nest/contact` | Protected — contact form → Mailpit |
| `http://localhost/nest/api/v1/status` | Public API |
| `http://localhost/nest/api/v1/ping` | Public API |
| `http://localhost/nest/api/v1/auth/token` | POST — issue a Bearer token |
| `http://localhost/nest/api/v1/me` | GET — private, Bearer token required |
| `http://localhost/nest/api/v1/token` | DELETE — revoke all tokens |
| `http://localhost/nest/api/v1/docs` | Swagger UI |
...
| `logs/nest/` | `app.log` — NestJS application log (winston) |
...
just logs-nest-app      # tail NestJS application log
```
...
just logs-seed-django    tail Python application log
just logs-nest           follow NestJS container stdout
just logs-nest-app       tail NestJS application log
